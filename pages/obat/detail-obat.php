...
if(isset($_GET['id'])) {
    ...
    $query = "SELECT p.id, p.nama_obat, p.kode_obat, p.stock, p.harga, p.keterangan, k.nama_kategori, s.nama_satuan, sup.nama_sup 
                FROM produk as p 
    ...
    if(mysqli_num_rows($result) > 0) {
        ...
        $response['id'] = $row['id'];
        $response['kode_obat'] = $row['kode_obat'];
        $response['nama_obat'] = $row['nama_obat'];
